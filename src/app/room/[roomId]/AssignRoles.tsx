  ...
  return(
    ...
    <div className="w-full h-full flex flex-col pr-3 custom-scrollbar overflow-y-scroll">
      <div className='flex flex-row gap-3'>
        <p className="text-xl lg:text-3xl text-[#9247E3]">Total Players : {totalPlayers}</p>
        {!isEnough && <p className="text-xl lg:text-3xl text-warning">Not enough players!</p>}
      </div>
      {Object.entries(script.roleList).map(([type, value], index) => (
        <div className="py-2" key={type}>
          <div className='flex flex-row gap-5'>
            <p className="text-xl lg:text-3xl">{type}</p>
            <p className="text-xl lg:text-3xl">{selectedRoles.filter(r => r.type == type.toLowerCase()).length} / {isEnough && script.numberTable[totalPlayers] ? script.numberTable[totalPlayers][index] : 0}</p>
          </div>
          ...
          <div className="flex flex-wrap w-full text-2xl gap-5 p-5">
            {value.roles.map((role, index) => (
              <Card
                ...
                className={`flex flex-col justify-center items-center w-[90px] h-[90px] lg:w-[120px] lg:h-[120px] p-1 relative cursor-pointer ${selectedRoles.some(r => r.name === role.name) ? '' : 'opacity-50'}`}
                bgColor={value.color}
              >
                <Image
                ...
                />
                <p className="text-xs lg:text-lg text-center text-black">{role.name}</p>
              </Card>
            ))}
      ...
      <div className='w-full flex flex-row justify-center items-center gap-5 pt-2'>
        <Button 
          className='flex flex-row text-xl lg:text-3xl justify-center items-center gap-2'
          onClick={assignRoles}>
          <svg className='w-[22px] lg:w-[30px]' width="31" height="30" viewBox="0 0 31 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <Button 
          ...
          <svg className='w-[18px] lg:w-[25px]' width="25" height="26" viewBox="0 0 25 26" fill="none" xmlns="http://www.w3.org/2000/svg">
