...
export default function ChatBox({ roomId, player }: ChatBoxProps) {
  ...
  return (
    <div className="p-2 w-full">
      <div ref={scrollContainerRef} className="h-[200px] w-[250px] lg:h-[300px] lg:w-[400px] custom-scrollbar overflow-y-scroll mb-2 p-1">
        {chat.map((msg, idx) => (
          <div key={idx} className="flex flex-row w-full gap-1 mb-1">
            <MessageBubble text={msg.text} isSender={msg.senderId == player.id} senderName={msg.senderName} />
          </div>
        ))}
        <div ref={bottomRef} />
      </div>
      <div className='w-full flex items-center gap-2 px-1 lg:px-2'>
        <TextInput
          ...
          className='flex-grow text-sm lg:text-lg'
          placeholder="Type a message..."
        />
        <svg 
          onClick={() => sendMessage()}
          className='cursor-pointer w-[25px] lg:w-[34px]' 
